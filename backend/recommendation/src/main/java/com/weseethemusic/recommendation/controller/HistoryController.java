...
public class HistoryController {
    ...
    // 음악 재생 기록 조회
    @GetMapping("/history")
    public ResponseDto<List<MusicDto>> getPlayHistory(@RequestHeader("X-Member-Id") Long memberId) {
    ...
    // 재생 기록 추가
    @PostMapping("/{musicId}")
    public ResponseDto<Void> addPlayHistory(@RequestHeader("X-Member-Id") Long memberId,
        @PathVariable("musicId") Long musicId) {
        try {
            historyService.addPlayHistory(memberId, musicId);
        } catch (Exception e) {
            throw new CustomException(ErrorCode.INTERNAL_SERVER_ERROR, "내부 서버 오류");
        }
